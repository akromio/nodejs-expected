...
suite(__filename, () => {
  {
    suite("lessThanOrEqualTo()", () => {
      {
        test("when values are less than or equal to, wrapper must be returned", () => {
          {
            const w = expected({
              'x': 0,
              'y': 1,
        ...
        test("when some value is greater than, assertion error must be raised", () => {
          {
            const out = _core.dogma.peval(() => {
              return expected({
                ...
                'y': 1,
                'z': 2
              }).members("x", "y").lessThanOrEqualTo(0);
            });
